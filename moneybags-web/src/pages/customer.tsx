...
import { canonicalCreditCardAccountReference, formatDate, isBillPayable, paymentOutcome } from "../utils";
...
function formattedAmount(value?: number): string { return value == null ? "Not specified" : `₹${Number(value).toLocaleString("en-IN")}`; }

function ProductConditions({ product, brief = false }: { product: Product; brief?: boolean }) {
  const eligibility = product.eligibilityRules?.find(rule => rule.active !== false);
  const card = product.creditCardRule;
  const interest = product.interestRule?.annualInterestRate;
  const age = eligibility?.minimumAge == null ? undefined : eligibility.maximumAge == null
    ? `${eligibility.minimumAge}+ years` : `${eligibility.minimumAge}–${eligibility.maximumAge} years`;
  const income = eligibility?.minimumMonthlyIncome == null ? undefined : `${formattedAmount(eligibility.minimumMonthlyIncome)} monthly income`;
  const requirements = [age, income, eligibility?.kycRequired ? "Approved KYC" : undefined].filter(Boolean);
  return <div class={`mb-product-conditions ${brief ? "is-brief" : ""}`}>
    <div><span>Eligibility</span><strong>{requirements.length ? requirements.join(" · ") : "See product policy"}</strong></div>
    {card && <>
      <div><span>Credit limit</span><strong>{formattedAmount(card.minimumCreditLimit)} – {formattedAmount(card.maximumCreditLimit)}</strong></div>
      <div><span>Interest-free period</span><strong>{card.interestFreeDays == null ? "Not specified" : `${card.interestFreeDays} days`}</strong></div>
      {!brief && <>
        <div><span>Minimum bill payment</span><strong>{card.minimumPaymentPercentage == null ? "Not specified" : `${card.minimumPaymentPercentage}% or ${formattedAmount(card.minimumPaymentAmount)}, whichever is higher`}</strong></div>
        <div><span>Bill due date</span><strong>{card.paymentDueDays == null ? "Not specified" : `${card.paymentDueDays} days after statement`}</strong></div>
        <div><span>Cash advance</span><strong>{card.cashAdvanceAllowed ? `Available up to ${card.cashAdvanceLimitPercentage ?? 0}% of the limit` : "Not available"}</strong></div>
      </>}
    </>}
    {!card && <>
      <div><span>Opening amount</span><strong>{formattedAmount(product.amountRule?.minimumOpeningBalance ?? product.amountRule?.minimumAmount)}</strong></div>
      {interest != null && <div><span>Annual interest rate</span><strong>{interest}%</strong></div>}
    </>}
    {card && interest != null && <div><span>Annual purchase interest</span><strong>{interest}%</strong></div>}
  </div>;
}
...
export function ProductsPage() {
  ...
  return <><PageHeader title="Explore products" description="Compare eligibility and key terms before you apply."/><div class="mb-product-grid">{products.map((product) => <article class="mb-product-card" key={product.productCode}><span class="mb-product-type">{product.subtype?.replaceAll("_", " ")}</span><h2>{product.productName}</h2><p>{product.description ?? "A MoneyBags product with clear terms and transparent pricing."}</p><ProductConditions product={product} brief/><details class="mb-product-details"><summary>View eligibility &amp; key terms</summary><ProductConditions product={product}/><p class="mb-terms-note">Final eligibility is checked against your current profile when you submit.</p></details><button class="mb-button mb-button-primary" onClick={() => navigate(product.category === "CREDIT_CARD" ? `/app/cards/apply?product=${product.productCode}` : product.subtype === "FIXED_DEPOSIT" ? `/app/fixed-deposits/open?product=${product.productCode}` : `/app/accounts/open?product=${product.productCode}`)}>Choose product</button></article>)}</div>{products.length === 0 && <EmptyState title="No active products" message="Product Master has not published an active product yet."/>}</>;
}
...
export function CardApplyPage(){
  ...
  return <><PageHeader title="Apply for a credit card" description="Review eligibility and key terms, then request a limit within the allowed range."/><Panel><form class="mb-form" onSubmit={submit}>{error&&<ErrorState error={error}/>}<label class="mb-field"><span>Credit-card product *</span><select required value={productCode} onChange={event=>chooseProduct((event.currentTarget as HTMLSelectElement).value)}>{products.map(product=><option value={product.productCode}>{product.productName} · {product.productCode}</option>)}</select></label>{selected&&<section class="mb-card-terms" aria-label="Card eligibility and key terms"><h2>Before you apply</h2><p>Make sure your current profile meets these conditions. The bank verifies them when your application is submitted.</p><ProductConditions product={selected}/></section>}<Field label="Requested credit limit" name="limit" type="number" value={limit} min={String(selected?.creditCardRule?.minimumCreditLimit??0.01)} max={selected?.creditCardRule?.maximumCreditLimit==null?undefined:String(selected.creditCardRule.maximumCreditLimit)} step="0.01" required hint={selected?.creditCardRule?`You can request between ₹${selected.creditCardRule.minimumCreditLimit?.toLocaleString("en-IN")} and ₹${selected.creditCardRule.maximumCreditLimit?.toLocaleString("en-IN")}.`:undefined} onInput={setLimit}/><button class="mb-button mb-button-primary" disabled={busy||!productCode}>{busy?"Submitting…":"Submit for review"}</button></form></Panel></>;
}
...
export function PaymentsPage(){
  ...
  if(remote.error)return <ErrorState error={remote.error} retry={remote.retry}/>;
  return <><PageHeader title="Payments" description="Track transfers, card purchases, bill repayments and deposit funding." action={<div class="mb-row-actions"><button onClick={()=>navigate("/app/payments/repay")}>Repay card bill</button><button onClick={()=>navigate("/app/payments/merchant")}>Card purchase</button><button class="mb-button mb-button-primary" onClick={()=>navigate("/app/payments/transfer")}>Transfer money</button></div>}/><Panel><PaymentRows payments={items(remote.data)}/></Panel></>;
}
...
export function TransferPage(){
  ...
  const[values,setValues]=useState<Record<string,string>>({sourceAccountId:"",targetAccountId:"",amount:"",currencyCode:"INR",reference:""});const[recipientMode,setRecipientMode]=useState<"own"|"other">("other");const[recipientNumber,setRecipientNumber]=useState("");const[recipient,setRecipient]=useState<TransferRecipient>();const[lookingUp,setLookingUp]=useState(false);const[reviewing,setReviewing]=useState(false);const[receipt,setReceipt]=useState<Payment>();const[error,setError]=useState<unknown>();const[busy,setBusy]=useState(false);const commands=useIdempotencyKeyStore();const set=(key:string)=>(value:string)=>{setValues(current=>({...current,[key]:value}));setReviewing(false);};
  ...
  const lookupRecipient=async()=>{const accountNumber=recipientNumber.trim();if(!accountNumber)return;setLookingUp(true);setError(undefined);setRecipient(undefined);setValues(current=>({...current,targetAccountId:""}));setReviewing(false);try{const result=await services.accounts.lookupTransferRecipient(accountNumber);setRecipient(result);setValues(current=>({...current,targetAccountId:result.accountId,currencyCode:result.currency}));}catch(reason){setError(reason);}finally{setLookingUp(false);}};
  ...
  if(!active.length)return <><PageHeader title="Transfer money" description="Send money from any active Savings or Current account."/><Panel><EmptyState title="No active account available" message="Open or activate a Savings/Current account before sending money."/></Panel></>;
  const destinationLabel=recipientMode==="other"?recipient?.maskedAccountNumber:active.find(account=>account.accountId===values.targetAccountId)?.maskedAccountNumber;
  return <><PageHeader title="Transfer money" description="Choose a source account, then send to your account or another MoneyBags customer."/><Panel><form class="mb-form" onSubmit={submit}>{error&&<ErrorState error={error}/>}<div class="mb-form-grid"><SelectField label="From account" value={values.sourceAccountId} onChange={set('sourceAccountId')}><option value="">Choose source account</option>{active.map(account=><option value={account.accountId}>{account.maskedAccountNumber} · {account.productName} · ₹{Number(account.availableBalance).toLocaleString('en-IN')}</option>)}</SelectField><SelectField label="Recipient type" value={recipientMode} onChange={value=>{const mode=value as "own"|"other";setRecipientMode(mode);setRecipient(undefined);setValues(current=>({...current,targetAccountId:""}));setReviewing(false);}}><option value="other">Another MoneyBags customer</option><option value="own">My other account</option></SelectField></div>{recipientMode==="own"?<SelectField label="To account" value={values.targetAccountId} onChange={set('targetAccountId')}><option value="">Choose destination account</option>{active.filter(account=>account.accountId!==values.sourceAccountId).map(account=><option value={account.accountId}>{account.maskedAccountNumber} · {account.productName}</option>)}</SelectField>:<><div class="mb-form-grid"><Field label="Recipient account number" name="recipientAccountNumber" value={recipientNumber} required onInput={value=>{setRecipientNumber(value);setRecipient(undefined);setValues(current=>({...current,targetAccountId:""}));setReviewing(false);}}/><div class="mb-field"><label>Verify recipient</label><button type="button" class="mb-button mb-button-secondary" disabled={lookingUp||!recipientNumber.trim()} onClick={lookupRecipient}>{lookingUp?"Verifying…":"Verify account"}</button></div></div>{recipient&&<div class="mb-quote"><div><span>Recipient account</span><strong>{recipient.maskedAccountNumber}</strong></div><div><span>Product</span><strong>{recipient.productName}</strong></div><div><span>Transfer currency</span><strong>{recipient.currency}</strong></div></div>}</>}<div class="mb-form-grid"><Field label="Amount" name="amount" type="number" value={values.amount} min="0.01" step="0.01" required onInput={set('amount')}/><Field label="Currency" name="currency" value={values.currencyCode} required onInput={set('currencyCode')}/></div><Field label="Reference" name="reference" value={values.reference} onInput={set('reference')}/>{reviewing&&<div class="mb-quote"><div><span>From</span><strong>{active.find(account=>account.accountId===values.sourceAccountId)?.maskedAccountNumber}</strong></div><div><span>To</span><strong>{destinationLabel}</strong></div><div><span>Amount</span><strong><Money value={Number(values.amount)} currency={values.currencyCode}/></strong></div></div>}<button class="mb-button mb-button-primary" disabled={busy||!values.sourceAccountId||!values.targetAccountId||values.sourceAccountId===values.targetAccountId||!values.amount}>{busy?"Submitting securely…":reviewing?"Confirm transfer":"Review transfer"}</button></form></Panel></>;
}
...
export function BillsPage({ repaymentOnly = false }: { repaymentOnly?: boolean } = {}) {
  const remote=useRemote((signal)=>services.bills.list(undefined,signal),[]);
  if(remote.loading)return <Loading label="Loading bills"/>;
  if(remote.error)return <ErrorState error={remote.error} retry={remote.retry}/>;
  const allBills=items<Bill>(remote.data);
  const bills=repaymentOnly?allBills.filter(bill=>isBillPayable(bill.status,bill.outstandingAmount)):allBills;
  return <><PageHeader title={repaymentOnly?"Credit-card repayments":"Bills"} description={repaymentOnly?"Choose a generated bill, then pay from an active Savings or Current account.":"Credit-card statements and amounts due."} action={repaymentOnly?<button class="mb-button mb-button-secondary" onClick={()=>navigate("/app/bills")}>Billing history</button>:undefined}/><div class="mb-account-grid">{bills.map(bill=><article class="mb-bill-card" key={bill.billId}><div><span>{bill.billingPeriod}</span><Status value={bill.status}/></div><strong><Money value={bill.outstandingAmount} currency={bill.currency}/></strong><small>Due {date(bill.paymentDueDate)} · minimum <Money value={bill.minimumAmountDue} currency={bill.currency}/></small><button class="mb-button mb-button-primary" disabled={!isBillPayable(bill.status,bill.outstandingAmount)} onClick={()=>navigate(`/app/bills/${bill.billId}/pay`)}>{isBillPayable(bill.status,bill.outstandingAmount)?"Pay bill":"No payment due"}</button></article>)}</div>{!bills.length&&<Panel><EmptyState title={repaymentOnly?"No bills need payment":"No bills available"} message={repaymentOnly?"Generated, partially paid, or overdue bills with an outstanding balance will appear here.":"Generated card bills will appear here."}/></Panel>}</>;
}

export function RepaymentsPage(){return <BillsPage repaymentOnly/>;}

export function BillPayPage({billId}:{billId:string}){
  const id=customerId();
  const remote=useRemote(async signal=>{const[bill,accounts]=await Promise.all([services.bills.one(billId,signal),id?services.accounts.list(id,signal):Promise.resolve({content:[],page:0,size:0,totalElements:0})]);return{bill,accounts};},[billId,id]);
  const[source,setSource]=useState('');const[amountMode,setAmountMode]=useState<"minimum"|"full"|"custom">("full");const[amount,setAmount]=useState('');const[reference,setReference]=useState('');const[reviewing,setReviewing]=useState(false);const[receipt,setReceipt]=useState<Payment>();const[error,setError]=useState<unknown>();const[busy,setBusy]=useState(false);const commands=useIdempotencyKeyStore();
  if(remote.loading)return <Loading label="Loading repayment details"/>;
  if(remote.error)return <ErrorState error={remote.error} retry={remote.retry}/>;
  const{bill,accounts}=remote.data!;
  const outstanding=Number(bill.outstandingAmount);
  const minimum=Math.min(Number(bill.minimumAmountDue)>0?Number(bill.minimumAmountDue):outstanding,outstanding);
  const selectedAmount=amountMode==="minimum"?minimum:amountMode==="full"?outstanding:Number(amount);
  const funding=items<Account>(accounts).filter(account=>account.status==='ACTIVE'&&account.productSubtype!=='FIXED_DEPOSIT'&&account.currency===bill.currency);
  const selectedAccount=funding.find(account=>account.accountId===source);
  const validationMessage=!source?"Choose the Savings or Current account to pay from.":!Number.isFinite(selectedAmount)||selectedAmount<=0?"Enter a payment amount greater than zero.":selectedAmount>outstanding?"The repayment cannot exceed the current outstanding amount.":selectedAccount&&selectedAmount>Number(selectedAccount.availableBalance)?"The selected account does not have enough available balance.":undefined;
  const change=()=>{setReviewing(false);setError(undefined);};
  const selectMode=(mode:"minimum"|"full"|"custom")=>{setAmountMode(mode);if(mode==="minimum")setAmount(String(minimum));if(mode==="full")setAmount(String(outstanding));if(mode==="custom")setAmount("");change();};
  const pay=async(event:Event)=>{event.preventDefault();if(!id||validationMessage)return;if(!reviewing){setReviewing(true);return;}setBusy(true);setError(undefined);try{const body={requestorCustomerId:Number(id),billId:bill.billId,sourceDepositAccountId:source,creditCardAccountId:canonicalCreditCardAccountReference(bill.accountId),amount:selectedAmount,currencyCode:bill.currency,reference:reference.trim()||`Bill ${bill.billingPeriod} repayment`};setReceipt(await services.payments.repay(body,commands.keyFor(body)));commands.reset();}catch(reason){setError(reason);}finally{setBusy(false);}};
  if(receipt){const outcome=paymentOutcome(receipt.status);const title=outcome==="success"?"Repayment completed":outcome==="pending"?"Repayment is being finalised":"Repayment could not be completed";return <Receipt title={title} reference={receipt.paymentId} tone={outcome} doneLabel="View payment details" done={()=>navigate(`/app/payments/${receipt.paymentId}`)}><div class="mb-repayment-result"><strong><Money value={receipt.amount} currency={receipt.currencyCode}/></strong><Status value={receipt.status}/>{receipt.status==="PENDING_BILLING"&&<p>Your funds and card balance were updated. Billing confirmation is pending and will be retried safely; do not submit this repayment again.</p>}{outcome==="error"&&<p>{receipt.failureMessage||"No money was marked as successfully repaid. Open payment details for the failure reason."}</p>}{outcome==="success"&&<p>The deposit account, card balance, and bill were settled successfully.</p>}<div class="mb-row-actions"><button type="button" onClick={()=>navigate("/app/bills")}>Billing history</button><button type="button" onClick={()=>navigate("/app/payments/repay")}>Other bills</button></div></div></Receipt>}
  if(!isBillPayable(bill.status,bill.outstandingAmount))return <><PageHeader title="Pay card bill" description={`${bill.billingPeriod} · ${date(bill.paymentDueDate)}`}/><Panel><EmptyState title="This bill does not need a repayment" message={`Its current status is ${bill.status.replaceAll("_"," ")} and there is no payable outstanding amount.`}/><button class="mb-button mb-button-secondary mb-center-action" onClick={()=>navigate("/app/bills")}>Return to billing history</button></Panel></>;
  return <><PageHeader title="Pay card bill" description={`${bill.billingPeriod} · due ${date(bill.paymentDueDate)}`} action={<Status value={bill.status}/>}/><div class="mb-summary-row"><article><span>Outstanding</span><strong><Money value={outstanding} currency={bill.currency}/></strong><small>Total due <Money value={bill.totalAmountDue} currency={bill.currency}/></small></article><article><span>Minimum amount due</span><strong><Money value={minimum} currency={bill.currency}/></strong><small>Pay by {date(bill.paymentDueDate)}</small></article><article><span>Already paid</span><strong class="mb-success-text"><Money value={bill.paidAmount} currency={bill.currency}/></strong><small>Card {bill.accountId}</small></article></div><Panel title={reviewing?"Review and confirm repayment":"Choose repayment details"}>{error&&<ErrorState error={error}/>} {!funding.length?<EmptyState title="No eligible funding account" message={`An active ${bill.currency} Savings or Current account is required to repay this bill.`}/>:<form class="mb-form" onSubmit={pay}><SelectField label="Pay from account" value={source} required onChange={value=>{setSource(value);change();}}><option value="">Choose an active account</option>{funding.map(account=><option value={account.accountId}>{account.maskedAccountNumber} · {account.productName} · available {account.currency} {Number(account.availableBalance).toLocaleString('en-IN')}</option>)}</SelectField><div class="mb-repayment-options" role="group" aria-label="Repayment amount"><button type="button" class={amountMode==="minimum"?"is-selected":""} onClick={()=>selectMode("minimum")}><span>Minimum due</span><strong><Money value={minimum} currency={bill.currency}/></strong></button><button type="button" class={amountMode==="full"?"is-selected":""} onClick={()=>selectMode("full")}><span>Full outstanding</span><strong><Money value={outstanding} currency={bill.currency}/></strong></button><button type="button" class={amountMode==="custom"?"is-selected":""} onClick={()=>selectMode("custom")}><span>Custom amount</span><strong>Choose another amount</strong></button></div>{amountMode==="custom"&&<Field label="Custom payment amount" name="amount" type="number" value={amount} min="0.01" max={String(outstanding)} step="0.01" required hint={`Maximum payable: ${bill.currency} ${outstanding.toLocaleString("en-IN")}`} onInput={value=>{setAmount(value);change();}}/>}<Field label="Payment reference (optional)" name="reference" value={reference} placeholder={`Bill ${bill.billingPeriod} repayment`} onInput={value=>{setReference(value);change();}}/>{validationMessage&&<p class="mb-field-error">{validationMessage}</p>}{reviewing&&<div class="mb-quote"><div><span>Bill</span><strong>{bill.billId}</strong></div><div><span>Funding account</span><strong>{selectedAccount?.maskedAccountNumber}</strong></div><div><span>Card account</span><strong>{canonicalCreditCardAccountReference(bill.accountId)}</strong></div><div><span>Repayment</span><strong><Money value={selectedAmount} currency={bill.currency}/></strong></div><div><span>Bill remaining after payment</span><strong><Money value={outstanding-selectedAmount} currency={bill.currency}/></strong></div></div>}<div class="mb-wizard-actions">{reviewing&&<button type="button" onClick={()=>setReviewing(false)}>Back</button>}<button class="mb-button mb-button-primary" disabled={busy||Boolean(validationMessage)}>{busy?"Submitting securely…":reviewing?"Confirm repayment":"Review repayment"}</button></div></form>}</Panel></>;
}
...
export function StatementsPage(){
  ...
  const cardsRemote=useRemote(signal=>id?services.cards.accounts(id,signal):Promise.resolve([]),[id]);
  ...
  const cards=(cardsRemote.data??[]).filter(card=>card.status!=="CLOSED");
  const statementAccounts=[...accounts.map(account=>({reference:account.accountId,label:`${account.maskedAccountNumber} · ${account.productName}`})),...cards.map(card=>({reference:`CC-${card.accountId}`,label:`${card.cardNumber||`Card ${card.accountId}`} · ${card.productCode} credit card`}))];
  useEffect(()=>{
    if(accountId||!statementAccounts.length)return;
    const requested=statementAccounts.find(account=>account.reference===requestedAccount);
    setAccountId((requested??statementAccounts[0]).reference);
  },[accountsRemote.data,cardsRemote.data,accountId,requestedAccount]);
