...
| internal | `POST` | `/internal/v1/deposit-accounts/eod/accruals` | Run daily CASA accrual controls; currency is required | `CasaAccrualRequest -> CasaAccrualResponse` |
| internal | `GET` | `/internal/v1/deposit-accounts/eod/readiness` | Report account blockers for daily close | `- -> ServiceReadinessResponse` |
| internal | `POST` | `/internal/v1/deposit-accounts/eod/fixed-deposit-accruals` | Run daily FD interest accrual; Idempotency-Key is required | `EodPeerCommand -> EodResult` |
| internal | `POST` | `/internal/v1/deposit-accounts/eod/fixed-deposit-maturities` | Process FDs maturing on the business date; Idempotency-Key is required | `EodPeerCommand -> EodResult` |
| internal | `GET` | `/internal/v1/deposit-accounts/eod/fixed-deposit-readiness` | Report FD funding and payout blockers | `- -> FixedDepositReadinessResponse` |
...
| internal | `POST` | `/internal/v1/accounting/fixed-deposit-reconciliation` | Reconcile FD accrual and maturity postings | `EodPeerCommand -> FixedDepositReconciliationResponse` |
...
Coordinates the business date and ordered synchronous end-of-day controls across Payments, Deposit and Fixed Deposit, Credit Card, Billing, Accounting, Statements, and Notifications.
