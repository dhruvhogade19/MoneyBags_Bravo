...
public class SecurityConfig {
    ...
    SecurityFilterChain secured(HttpSecurity http) throws Exception {
        return http.csrf(csrf -> csrf.disable()).authorizeHttpRequests(auth -> auth
                ...
                .requestMatchers("/internal/**").hasAuthority("SCOPE_payment:service")
                .requestMatchers(HttpMethod.GET, "/api/v1/payments/**")
                    .hasAnyAuthority("SCOPE_payment:read", "SCOPE_payment:admin")
                .requestMatchers(HttpMethod.POST, "/api/v1/payments/**")
                    .hasAnyAuthority("SCOPE_payment:write", "SCOPE_payment:admin")
                .anyRequest().denyAll())
