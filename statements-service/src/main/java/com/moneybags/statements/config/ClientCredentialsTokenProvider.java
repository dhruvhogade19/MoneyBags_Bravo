...
public class ClientCredentialsTokenProvider {
    ...
    public ClientCredentialsTokenProvider(
            ...
            @Value("${moneybags.security.m2m.client-id:statements-service}") String clientId,
            @Value("${moneybags.security.m2m.client-secret:${M2M_CLIENT_SECRET:}}") String secret,
            @Value("${moneybags.security.m2m.scope:account:service accounting:service}") String scope) {
    ...
    public String token() {
        ...
        if (secret.isBlank()) throw new IllegalStateException(
                "M2M_CLIENT_SECRET is required when security is enabled");
        CachedToken value = cached;
        if (valid(value)) return value.value();
        synchronized (this) {
            value = cached;
            if (valid(value)) return value.value();
            ...
            if (response == null || response.access_token() == null) {
                throw new IllegalStateException("Identity service returned no access token");
            }
            cached = new CachedToken(response.access_token(),
                    Instant.now().plusSeconds(Math.max(60, response.expires_in())));
            return cached.value();
        }
    }

    private boolean valid(CachedToken value) {
        return value != null && value.expiresAt().isAfter(Instant.now().plusSeconds(30));
    }
